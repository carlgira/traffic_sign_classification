...
from src.capsnet.config import cfg
...
class CapsLayer(object):
    ...
    Returns:
        A 4-D tensor.
    '''
    def __init__(self, num_outputs, conv_vec_len, fcc_vec_len, caps_neurons, with_routing=True, layer_type='FC'):
    ...
    def __call__(self, input, kernel_size=None, stride=None):
        ...
        if self.layer_type == 'FC':
            if self.with_routing:
                ...
                self.input = tf.reshape(input, shape=(cfg.batch_size, self.caps_neurons, 1, self.conv_vec_len, 1))

                with tf.variable_scope('routing'):
                    # b_IJ: [1, 1, num_caps_l, num_caps_l_plus_1, 1]
                    b_IJ = tf.zeros(shape=[1, self.caps_neurons, self.num_outputs, 1, 1], dtype=np.float32)
                    capsules = self.routing(self.input, b_IJ, self.caps_neurons)
                    capsules = tf.squeeze(capsules, axis=1)

            return(capsules)


    def routing(self, input, b_IJ, caps_neurons):
        ...
        W = tf.get_variable('Weight', shape=(1, caps_neurons, self.num_outputs, self.conv_vec_len, self.fcc_vec_len), dtype=tf.float32,
        ...
        print("routing.input.shape", input.get_shape())

        # in last 2 dims:
        # [8, 16].T x [8, 1] => [16, 1] => [batch_size, 1152, 10, 16, 1]
        u_hat = tf.matmul(W, input, transpose_a=True)
        #assert u_hat.get_shape() == [cfg.batch_size, 1152, 10, 16, 1]
        print("routing.u_hat.shape", u_hat.get_shape())
        ...
        for r_iter in range(cfg.iter_routing):
            with tf.variable_scope('iter_' + str(r_iter)):
                ...
                print("routing.cIJ.shape", c_IJ.get_shape())
                ...
                print("routing.s_J.shape", s_J.get_shape())

                # line 6:
                # squash using Eq.1,
                v_J = squash(s_J)
                #assert v_J.get_shape() == [cfg.batch_size, 1, 10, 16, 1]
                print("routing.v_J.shape", v_J.get_shape())

                # line 7:
                # reshape & tile v_j from [batch_size ,1, 10, 16, 1] to [batch_size, 10, 1152, 16, 1]
                # then matmul in the last tow dim: [16, 1].T x [16, 1] => [1, 1], reduce mean in the
                # batch_size dim, resulting in [1, 1152, 10, 1, 1]
                v_J_tiled = tf.tile(v_J, [1, caps_neurons, 1, 1, 1])
