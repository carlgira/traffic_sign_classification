...
class ConvNet(tsf.TrafficSignClassifier):
	...
	def neural_network(self, x_data, y_data, phase):
		...
		conv2 = tf.layers.conv2d(conv1, filters=32, kernel_size=[3, 3], padding='valid', activation=tf.nn.relu)
		conv2 = tf.layers.max_pooling2d(conv2, pool_size=[2, 2], strides=2)

		conv3 = tf.layers.conv2d(conv2, filters=64, kernel_size=[3, 3], padding='valid', activation=tf.nn.relu)
